import { UserListResponse, UserStatus } from '@/api/system/user';
...
export const getColumns = (props: ConfigType): TableColumnsType<UserListResponse> => [
  ...
  },
  {
    ...
    dataIndex: 'statusStr',
    key: 'statusStr',
    render: (statusStr: number, record: UserListResponse) => <Tag color={record.status === UserStatus.NORMAL ? 'success' : 'error'}>{statusStr}</Tag>,
  },
  {
    title: '登录次数',
    width: 100,
    ...
    render: (_: any, record: UserListResponse) => (
      <Space size={15}>
        <WithTooltipConfirm tooltipTitle="编辑" showPopconfirm={false}>
          <Iconify size={21} className="cursor-pointer text-primary" icon="la:user-edit" onClick={() => props.handleEdit(record)} />
        </WithTooltipConfirm>
        <WithTooltipConfirm tooltipTitle="删除" popconfirmTitle="确定要删除吗？" onConfirm={() => props.handleDelete(record)}>
